...
char *C_HL_keywords[] = {"int|", "long|", "float|", "double|", "char|", "void|", "unsigned|", "signed|", "return", "if", "else", "for", "while", "do", "switch", "case", "break", "continue", "struct", "union", "enum", "typedef", "static", "extern", "inline", "volatile", "register", "const", "volatile", "restrict", NULL};
...
int editorRowRxToCx(erow *row, int rx) {
    ...
    for(cx = 0; cx < row->size; cx++) {
        if(row->chars[cx] == '\t'){
            cur_rx += (TTYNOTE_TAB_STOP - 1) - (cur_rx % TTYNOTE_TAB_STOP);
        }
        cur_rx++;
...
void editorUpdateRow(erow *row) {
    ...
    free(row->render);
    row->render = malloc(row->size + tabs*(TTYNOTE_TAB_STOP - 1) + 1);
...
void editorFind() {
    ...
    char *query = editorPrompt("Search: %s (ESC to cancel/ Arrows to navigate/ Enter to search)", editorFindCallback);
...
// this is a function to append the output data to the output buffer
...
void editorDrawsStatusBar(struct abuf *ab) {
...
void editorDrawsMessageBar(struct abuf *ab) {
    abAppend(ab, "\x1b[K", 3);
...
void editorRefreshScreen() {
    ...
    editorDrawRows(&ab);
    editorDrawsStatusBar(&ab);
    editorDrawsMessageBar(&ab);
